...
trait ProductFormats extends ProductFormatsInstances {
    ...
    def jsonFormat0[T](construct: () => T): RootJsonFormat[T] = new RootJsonFormat[T] {
    ...
    protected def fromField[T](value: JsValue, fieldName: String)(implicit reader: JsonReader[T]) = value match {
        case x: JsObject if(reader.isInstanceOf[OptionFormat[_]] & !x.fields.contains(fieldName)) => None.asInstanceOf[T]
        case x: JsObject => try {
            reader.read(x.fields(fieldName))
        }catch{
            case e: NoSuchElementException => deserializationError("Object is missing required member '" + fieldName + "'", e, fieldName :: Nil)
            case DeserializationException(msg, cause, fieldNames) => deserializationError(msg, cause, fieldName :: fieldNames)
        }
    ...
    protected def extractFieldNames(classTag: scala.reflect.ClassTag[_]): Array[String] = {
        ...
        try{
            ...
            if(fields.zip(copyDefaultMethods).exists { case (f, m) => f.getType != m.getReturnType }){
                sys.error("Can not determine field order of case class " + clazz.getName)
            }
            fields.map(f => ProductFormats.unmangle(f.getName))
        }catch{
            case NonFatal(ex) => throw new RuntimeException("Can not automatically determine case class field names and order " + "for '" + clazz.getName + "', please use the 'jsonFormat' overload with explicit field name specification", ex)
        }
    }
}
