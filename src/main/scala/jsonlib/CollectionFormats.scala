...
trait CollectionFormats {
     ...
     */
    implicit def arrayFormat[T : JsonFormat : scala.reflect.ClassTag] = new RootJsonFormat[Array[T]] {
    ...
    implicit def mapFormat[K : JsonFormat, V : JsonFormat] = new RootJsonFormat[Map[K, V]] {
        ...
        def read(value: JsValue) = value match {
            case x: JsObject => x.fields.map { field => (JsString(field._1).convertTo[K], field._2.convertTo[V]) }(collection.breakOut)
    ...
    implicit def immSetFormat[T : JsonFormat]           = viaSeq[Vector[T], T](seq => Vector(seq : _*))
