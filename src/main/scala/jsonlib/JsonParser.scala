...
object JsonParser {
    ...
    class ParsingException(val summary: String, val detail: String = "") extends RuntimeException(if(summary.isEmpty) detail else if(detail.isEmpty) summary else summary + ":" + detail)
}
...
class JsonParser(input: ParserInput){
    ...
    private def `number`() = {
        ...
        `frac`()
        `exp`()
        jsValue = if(startChar == '0' && input.cursor - start == 1) JsNumber.zero else JsNumber(input.sliceCharArray(start, input.cursor))
        ws()
    }
        ...
        def nextUtf8Char() = {
            ...
            if(charBuffer.position() > 0){
                ...
                if(_cursor < bytes.length){
                    ...
                    else if((byte & 0xF0) == 0xE0) decode(byte, 1) // 3-byte UTF-8 sequence
                    else if((byte & 0xF8) == 0xF0) decode(byte, 1) // 4-byte UTF-8 sequence
                    else ErrorChar
                } else {
                    EOI
                }
