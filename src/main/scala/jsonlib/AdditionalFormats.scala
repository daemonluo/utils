...
trait AdditionalFormats {
    ...
    def lift[T](writer: JsonWriter[T]) = new JsonFormat[T] {
        ...
        def read(value: JsValue) = throw new UnsupportedOperationException("JsonReader implementation missing")
    }

    /**
     * Turns a RootJsonWriter into a RootJsonFormat that throws an UnsupportedOperationException for reads.
     */
    def lift[T](writer: RootJsonWriter[T]): RootJsonFormat[T] = rootFormat(lift(writer: JsonWriter[T]))
    ...
    def lift[T <: AnyRef](reader: JsonReader[T]) = new JsonFormat[T] {
        def write(obj: T): JsValue = throw new UnsupportedOperationException("No JsonWriter[" + obj.getClass + "] available")
     ...
     */
    def lift[T <: AnyRef](reader: RootJsonReader[T]): RootJsonFormat[T] = rootFormat(lift(reader: JsonReader[T]))
