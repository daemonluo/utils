...
private[opt] case class OptionDefinition[C](
...
private[opt] class SeparatorDefinition[C](
    description: String
) extends OptionDefinition[C](false, null, null, null, null, description, {(a: String, c: C) => c}, false, false, 1, 1)

private[opt] class Argument[C](
    name: String,
    description: String,
    minOccurs: Int,
    maxOccurs: Int,
    action: (String, C) => C
) extends OptionDefinition[C](false, null, name, null, name, description, action, false, false, minOccurs, maxOccurs) {
    override def shortDescription = "argument " + name
}

private[opt] class ArgOptionDefinition[C](
    shortopt: Option[String],
    longopt: String,
    valueName: String,
    description: String,
    action: (String, C) => C
) extends OptionDefinition[C](true, shortopt, longopt, null, valueName, description, action, true, false, 0, UNBOUNDED)
 
private[opt] class IntArgOptionDefinition[C](
    shortopt: Option[String],
    longopt: String,
    valueName: String,
    description: String,
    action: (Int, C) => C
) extends OptionDefinition[C](true, shortopt, longopt, null, valueName, description, {(a: String, c: C) => action(a.toInt, c)}, true, false, 0, UNBOUNDED)

private[opt] class DoubleArgOptionDefinition[C](
    shortopt: Option[String],
    longopt: String,
    valueName: String,
    description: String,
    action: (Double, C) => C
) extends OptionDefinition[C](true, shortopt, longopt, null, valueName, description, {(a: String, c: C) => action(a.toDouble, c) }, true, false, 0, UNBOUNDED)

private[opt] class BooleanArgOptionDefinition[C](
    shortopt: Option[String],
    longopt: String,
    valueName: String,
    description: String,
    action: (Boolean, C) => C
) extends OptionDefinition[C](true, shortopt, longopt, null, valueName, description, { (a: String, c: C) =>
    val boolValue = a.toLowerCase match {
        ...
        case "1" => true
        case "0" => false
        case _ => throw new IllegalArgumentException("Expected a string can interpret as a boolean")
    }
    action(boolValue, c)
}, true, false, 0, UNBOUNDED)

private[opt] object KeyValueParser {
...
private[opt] class KeyValueArgOptionDefinition[C](
    shortopt: Option[String],
    longopt: String,
    keyName: String,
    valueName: String,
...
) extends OptionDefinition[C](true, shortopt, longopt, keyName, valueName, description, { (a: String, c: C) =>
    action(KeyValueParser.split(a)._1, KeyValueParser.split(a)._2, c)
}, false, true, 0, UNBOUNDED)

private[opt] class KeyIntValueArgOptionDefinition[C](
    shortopt: Option[String],
    longopt: String,
    keyName: String,
    valueName: String,
    ...
    action(KeyValueParser.split(a)._1, KeyValueParser.split(a)._2.toInt, c)
}, false, true, 0, UNBOUNDED)

private[opt] class KeyDoubleValueArgOptionDefinition[C](
    shortopt: Option[String],
    longopt: String,
    keyName: String,
    valueName: String,
    ...
    action(KeyValueParser.split(a)._1, KeyValueParser.split(a)._2.toDouble, c)
}, false, true, 0, UNBOUNDED)

private[opt] class KeyBooleanValueArgOptionDefinition[C](
    shortopt: Option[String],
    longopt: String,
    keyName: String,
    valueName: String,
...
private[opt] class FlagOptionDefinition[C](
    shortopt: Option[String],
    longopt: String,
    description: String,
    action: C => C
) extends OptionDefinition[C](true, shortopt, longopt, null, null, description, {(a: String, c: C) => action(c) }, false, false, 0, UNBOUNDED)

private[opt] trait GenericOptionParser[C]{
    ...
    private def descriptions: Seq[String] = opts.map(opt => {
        opt match {
    ...
    def usage : String = {
        val programText = programName map { _ + " " } getOrElse { "" }
        val versionText = programName map { pg => version map { NL + pg + " " + _ } getOrElse { "" } } getOrElse { "" }
        val optionText = if(opts.isEmpty){""}else{"[options] "}
        val argumentList = argumentNames.mkString(" ")

        versionText + NL + "Usage: " + programName + optionText + argumentList + NLNL + "  " + descriptions.mkString(NL + "  ") + NL 
    }
    ...
    private def applyArgument(option: OptionDefinition[C], arg: String, config: C): Option[C] = {
        try{
            ...
            case e: NumberFormatException =>
                util.error("Error: " + option.shortDescription + " expects a number but was given '" + arg + "'")
                None
            case e: Throwable =>
                util.error("Error: " + option.shortDescription + " failed when given '" + arg + "'." + e.getMessage)
                None
        }
    }
    ...
    def parse(args: Seq[String], init: C): Option[C] = {
        ...
        while(i < args.length){
            ...
            val matchingOption = opts.find( opt => {
                opt.canBeInvoked && (
                    (!opt.keyValueArgument && (arg == "--" + opt.longopt || (opt.shortopt map { o => arg == "-" + o } getOrElse { false }))) ||
                    (
                        opt.keyValueArgument && (
                            arg.startsWith("--" + opt.longopt + ":") || (opt.shortopt map { o => arg.startsWith("-" + o + ":") } getOrElse { false })
                        )
                    )
                )
            })
                ...
                case None =>
                    ...
                    if(arg.startsWith("-")){
                        if(errorOnUnknownArgument){
                            util.error("Error: Unknown argument '" + arg + "'")
                            _error = true
                        }else{
                            util.warn("Warning: Unknown argument '" + arg + "'")
                        }
                ...
                case Some(option) =>
                    val argToPass: String = if(option.gobbleNextArgument) {
                        ...
                        if(i >= args.length){
                            ...
                            if(errorOnUnknownArgument){
                                util.error("Error: missing value after '" + arg + "'")
                                _error = true
                            }else{
                                util.warn("Warning: missing value after '" + arg + "'")
                            }
                            ""
                        }else{
                            args(i)
                        }
                    }else if(option.keyValueArgument && (option.shortopt map { o => arg.startsWith("-" + o + ":") } getOrElse { false })){
        ...
        if((unseenArgs.toList exists { _.minOccurs > 0 }) || (argListCount == 0 && (argList match {
            case Some(a: Argument[C]) => a.minOccurs > 0
            case _ => false
        }))){
            util.error("Error: missing arguments: " + argumentNames.mkString(", "))
            _error = true
        }
        if(_error){
            util.error(usage)
